...
class RepositoryTest extends TestCase
{
    ...
    function testTeamsAndInsertTeam(): void
    {
        ...
        $this->assertEquals($this->repository->insertTeam($teams[9]), 10);
        $this->assertEquals($this->repository->insertTeam($teams[2]), 3);

        $this->assertEquals($this->repository->teams(), [$teams[2], $teams[4], $teams[9]]);
        $this->assertEquals($this->repository->teams()[0]['id'], 3);
    }
    ...
    {   
        ...
        $this->assertEquals($this->repository->insertTeam($teams[3]), 4);
        $this->assertEquals($this->repository->insertTeam($teams[17]), 18);

        $this->assertEquals($this->repository->insertTeam($teams[6]), 7);
        $this->assertEquals($this->repository->insertTeam($teams[18]), 19);
        
        $this->assertEquals($this->repository->insertMatch($matches[6]), 7);
        $this->assertEquals($this->repository->insertMatch($matches[0]), 1);
        
        $this->assertEquals($this->repository->matches(), [$matches[0], $matches[6]]);
        $this->assertEquals($this->repository->matches()[0]['id'], 1);
    }
